...
<title>需要审核的评论</title><link href="${contextPath}liger/lib/ligerUI/skins/Aqua/css/ligerui-all.css" rel="stylesheet" type="text/css" /> 
    ...
    <script type="text/javascript">   
        ...
        { 
            g = manager = $("#maingrid").ligerGrid({
                ...
                { display: '文章id', name: 'compostid', width: 140,hide: true},
                { display: '文章名称', name: 'arname', width: 80,hide: true},
                { display: '评论地址', name: 'arurl', width: 200, type: 'text',align: 'left',frozen:true ,render:function(rowdata, index, value){
                	return   "<a href='../archives/"+rowdata.arurl+"' targer='_black'>"+rowdata.arname+"</a>";  
                }},
                { display: '评论者', name: 'author',width: 100,align: 'left',frozen:true},
                { display: '评论ip', name: 'comip',width: 100,align: 'left',frozen:true},
                { display: '评论时间', name: 'comdate', width: 140, align: 'left',type: 'date',frozen:true,render:function(rowdata, index, value){
                ...
                { display: '评论内容', name: 'comcontent', width: 400, align: 'left',frozen:true}
                ],
                ...
                dataAction:'server',
                url:"commensdata.do?iden=0&method=getUserPaging&privilege=-1",
                usePager:true,
				pageSize:10,
                width: "100%"
            });
...
<body  style="padding:10px">
   ...
   <a class="l-button" style="width:120px" onclick="getSelected('1')">通过</a>
   <a class="l-button" style="width:120px" onclick="getSelected('-1')">标记为垃圾</a>
   <a class="l-button" style="width:120px" onclick="getSelected('-2')">放进回收站</a>
   <a class="l-button" style="width:120px" onclick="getSelected('-3')">彻底删除</a>
