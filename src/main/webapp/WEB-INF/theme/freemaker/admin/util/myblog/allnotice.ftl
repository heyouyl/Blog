    ...
    <script type="text/javascript">
        ...
        function f_initGrid()
        {
            g = manager = $("#maingrid").ligerGrid({
                columns: [
                { display: '公告id', name: 'arid', width: 50, type: 'int', frozen: true },
                { display: '公告名', name: 'arname',width: 380,
                    editor: { type: 'text' }
                },
                ...
                type:"post",
                url: "allnoticedata.do?method=getUserPaging&privilege=-1",
                pageSize:16 ,
                width: '100%'
            });   
        }
		...
		function editRow(rowid) {
			...
			if (confirm('进入新的tab页编辑')){
				f_addTabItem('addpage.html?arid='+arid,tname);
			}
        }
